        ...
        **kwargs : dict, optional
        ...
        if graph_features is not None:
            ...
            if isinstance(X_label_entire, pd.DataFrame):
                ...
                # When X is an array
                X_graph = X_label_entire[:, graph_features]
                X_graph_smote = X_label_smote[:, graph_features]
                X_ungraph = X_unlabel_entire[:, graph_features]
        ...
        for _ in range(self.max_iterations):          
            ...
            iid_observed = iid_observed[-L_.shape[0]:, :X_label_smote.shape[0]] #add this to compare against the labeled instances
            ...
            # Show the distribution of the labels in the new labeled set
            if (self.messages) and (iteration % self.view == 0):
                print(f"Distribution of labels in the new labeled set:")
                print(pd.Series(y_label).value_counts().sort_index())
                if self.y_real_label is not None:
            ...
            if self.htunning:
                ...
                X_retrain, X_reval, y_retrain, y_reval = train_test_split(X_label_smote, y_label_smote, 
                                                                          test_size=0.5, 
                                                                          random_state=random_state,

...
class Setred_scratch(BaseEstimator, MetaEstimatorMixin,ClassifierMixin):
    ...
    def fit(self, X, y, mod_features, graph_features , **kwargs):
        """ 
        ...
        mod_features : list of str with the names of the features to use for the model.
        graph_features : list of str with the names of the features to use for the graph.
        **kwargs : dict, optional
        ...
        if mod_features is not None:
            ...
            if isinstance(X_label_entire, pd.DataFrame):
                ...
                X_label_smote = X_label_entire_smote[mod_features]                
        ...
        for _ in range(self.max_iterations):
            ...
            pre_yL = np.concatenate((y_label_smote, y_), axis = 0)
            ...
            iid_observed = iid_observed[-L_.shape[0]:, :X_label_smote.shape[0]]
            ...
            weights = self.__create_neighborhood_knn(X_graph_smote, Lg_)
            ...
            if is_df:
                ...
                Lg_filtered = Lg_.iloc[to_add, :]
            else:
                L_filtered = L_[to_add, :]
                Lg_filtered = Lg_[to_add, :]
            ...
            if is_df:
                ...
                X_ungraph = X_ungraph.drop(index=X_ungraph.index[to_delete])
            else:
                X_unlabel = np.delete(X_unlabel, to_delete, axis=0)
                X_ungraph = np.delete(X_ungraph, to_delete, axis=0)
            ...
            if (self.messages) and (iteration % self.view == 0):
                logger.info("-----------------------------------------------------------------")
                logger.info(f"09-00 iteration {iteration}: Resampling the labeled instances with SMOTE.")
                logger.info("-----------------------------------------------------------------")
                logger.info(f"Distribution of the labels in the resampled labeled set:")
                logger.info(pd.Series(y_label_smote).value_counts().sort_index())
            
            # Check if the features are provided
            if mod_features is not None:
                # Check if the features are in the DataFrame
                if isinstance(X_label_entire_smote, pd.DataFrame):
                    X_label_smote = X_label_entire_smote[mod_features]
                else:
                    # When X is an array 
                    X_label_smote = X_label_entire_smote[:, mod_features]
            if graph_features is not None:
                # Check if the features are in the DataFrame
                if isinstance(X_label_smote, pd.DataFrame):
                    X_graph_smote = X_label_entire_smote[graph_features]
                else:
                    # When X is an array
                    X_graph_smote = X_label_entire_smote[:, graph_features]
            ...
            if self.htunning:
                ...
                X_retrain, X_reval, y_retrain, y_reval = train_test_split(X_label_smote, 
                                                                          y_label_smote, 
                                                                          test_size=0.5, 
                                                                          random_state=random_state,
                                                                          stratify=y_label_smote)
                ...
                self._base_estimator.fit(X_label_smote, y_label_smote, **kwargs)    # I, Juan Felipe, have added this line to retrain the base estimator in each iteration.
        ...
        self.X_label_entire_smote_ = X_label_entire_smote
        self.y_label_smote_ = y_label_smote
